...
# _C.MODEL.Feature_Match = False
# _C.MODEL.Feature_DA = False
...
# _C.SOLVER.MIDDLE_HEAD.PLABEL_TH = (0.5, 1.0)


# _C.MODEL.MIDDLE_HEAD = CN()
# _C.MODEL.MIDDLE_HEAD.NUM_CONVS_IN = 1
...
_C.SOLVER.MIDDLE_HEAD = CN()
_C.SOLVER.MIDDLE_HEAD.BASE_LR = 0.005
_C.SOLVER.MIDDLE_HEAD.STEPS =  (60000, )
_C.SOLVER.MIDDLE_HEAD.WARMUP_ITERS = 1000
_C.SOLVER.MIDDLE_HEAD.WARMUP_METHOD = "constant"
_C.SOLVER.MIDDLE_HEAD.PLABEL_TH = (0.5, 1.0)  # sample the node satisfying cls 0.5 < score < 1.0
_C.SOLVER.MIDDLE_HEAD.BIAS_LR_FACTOR = 2
_C.SOLVER.MIDDLE_HEAD.GAMMA = 0.1
_C.SOLVER.MIDDLE_HEAD.WARMUP_FACTOR = 1.0 / 3
...
_C.MODEL.MIDDLE_HEAD.CONDGRAPH_ON = False
_C.MODEL.MIDDLE_HEAD.IN_NORM = 'LN' # vision-to-graph transformation (support GN/BN/LN)
_C.MODEL.MIDDLE_HEAD.NUM_CONVS_IN = 1 # the number of conv/linear in V2G
_C.MODEL.MIDDLE_HEAD.GM.WITH_HyperGNN = False
_C.MODEL.MIDDLE_HEAD.GM.HyperEdgeNum = 9 # the number of nodes in a hyperedge
_C.MODEL.MIDDLE_HEAD.GM.NUM_HYPERGNN_LAYER = 1 # the numeber of HGCN layers (1/2)
_C.MODEL.MIDDLE_HEAD.GM.ANGLE_EPS = 1e-2 # \epsilon in  Eq.7
_C.MODEL.MIDDLE_HEAD.GM.NUM_NODES_PER_LVL_SR = 100  # per-level maximum nodes
_C.MODEL.MIDDLE_HEAD.GM.NUM_NODES_PER_LVL_TG = 100
_C.MODEL.MIDDLE_HEAD.GM.BG_RATIO = 8 # the background node ratio
_C.MODEL.MIDDLE_HEAD.GM.NODE_DIS_WEIGHT = 0.1 # node discriminator loss weight
_C.MODEL.MIDDLE_HEAD.GM.NODE_DIS_LAMBDA =  0.02 # gradient scaling
_C.MODEL.MIDDLE_HEAD.GM.NODE_LOSS_WEIGHT = 1.0 # node loss to train auxilary parameters
_C.MODEL.MIDDLE_HEAD.GM.MATCHING_LOSS_WEIGHT = 0.1 # for adapting p(x|y) with matching
_C.MODEL.MIDDLE_HEAD.GM.MATCHING_LOSS_CFG = 'MSE' # matching loss config
_C.MODEL.MIDDLE_HEAD.GM.MATCHING_CFG = 'o2o' # one-to-one (o2o) or many-to-many (m2m) (m2m replaces sinkhorn with sigmoid)
_C.MODEL.MIDDLE_HEAD.GM.NODE_DIS_PLACE = 'feat' # 'intra', 'inter'
_C.MODEL.MIDDLE_HEAD.GM.WITH_QUADRATIC_MATCHING = True # high-order matching constraints in graph matching.
_C.MODEL.MIDDLE_HEAD.GM.WITH_NODE_DIS = True # node discriminator for adapting p(x)
_C.MODEL.MIDDLE_HEAD.GM.WITH_SEMANTIC_COMPLETION = True # conduct Gaussian sampling to complete the missing nodes
_C.MODEL.MIDDLE_HEAD.GM.WITH_CLUSTER_UPDATE = False # use spectral clustering to update the Graph-guided Memory Bank (needs warm-up)
_C.MODEL.MIDDLE_HEAD.GM.WITH_COMPLETE_GRAPH = True # conduct one-layer graph-based message propagation
_C.MODEL.MIDDLE_HEAD.GM.WITH_DOMAIN_INTERACTION = True # Cross Graph Interaction
_C.MODEL.MIDDLE_HEAD.GM.WITH_SCORE_WEIGHT = False # use confidence as the adaptive weight in the node loss
_C.MODEL.MIDDLE_HEAD.GM.WITH_CTR = False  # sample target nodes with classification scores \times centerness scores
