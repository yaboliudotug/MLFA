...
class FCOSDiscriminator(nn.Module):
    def __init__(self, num_convs=2, in_channels=256, grad_reverse_lambda=-1.0, grl_applied_domain='both',patch_stride=None):
        ...
        self.ln1 = nn.Linear(in_channels, in_channels // 2) # 128
        self.ln2 = nn.Linear(in_channels // 2, in_channels // 4) # 64
        self.ln3 = nn.Linear(in_channels // 4, 2) # 2
    ...
    def forward(self, feature, domain='source'):
        ...
        x_s = self.ln3(self.ln2(self.ln1(features_s)))
        x_t = self.ln3(self.ln2(self.ln1(features_t)))
