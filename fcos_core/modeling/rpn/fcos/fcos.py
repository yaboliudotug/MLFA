...
class FCOSModule(torch.nn.Module):
    ...
    def forward(self, images, features, targets=None, return_maps=False, act_maps= None, for_tta=False):
        """
        Arguments:
        ...
        if for_tta:
            return self._forward_tta_target(
    ...
    def _forward_tta_target(self, box_cls, box_regression, centerness):
        scores = []
        for i, cls_logits in enumerate(box_cls):
            if self.with_ctr:
        ...
        losses = {
            "zero": 0.0 * sum(0.0 * torch.sum(x) for x in box_cls)
                    +0.0 * sum(0.0 * torch.sum(x) for x in box_regression)
                    +0.0 * sum(0.0 * torch.sum(x) for x in centerness)}
        return scores, losses, (box_cls, box_regression, centerness)
